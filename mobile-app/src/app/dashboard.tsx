...
export default function DashboardScreen() {
  ...
  if (error || !dashboardData) return <DemoDashboard error={error} logout={logout} onLocation={() => router.push('/location')} />;
  ...
  return <SafeAreaView style={styles.screen}><ScrollView contentContainerStyle={styles.content}><View style={styles.header}><ScreenHeader eyebrow="Farmer dashboard" title="AGRI BOT" actionLabel="Log out" onAction={logout} /><Text style={styles.tagline}>Smart farming, simplified.</Text></View><View style={styles.statusRow}><View style={styles.statusHeader}><View style={styles.liveLabel}><View style={[styles.statusDot, { backgroundColor: isSocketLive ? AgriColors.success : AgriColors.warning }]} /><Text style={[styles.liveText, { color: isSocketLive ? AgriColors.success : AgriColors.warning }]}>{isSocketLive ? 'LIVE' : socketStatus === 'connecting' ? 'CONNECTING' : 'OFFLINE'}</Text></View><Text style={[styles.statusValue, { color: isRobotOnline ? AgriColors.success : AgriColors.warning }]}>{robot.status.toUpperCase()}</Text></View><Text style={styles.statusLabel}>AgriBot {isRobotOnline ? 'connected' : 'offline'}</Text><Text style={styles.statusHint}>{socketStatusText}</Text></View><View style={styles.sectionHeading}><View><Text style={styles.sectionKicker}>FIELD CONDITIONS</Text><Text style={styles.sectionTitle}>Live environmental readings</Text></View><Text style={styles.updated}>{formatLastUpdated(robot.lastUpdated)}</Text></View><View style={styles.grid}><SensorCard accentColor={AgriColors.soil} detail="Simulated demo reading" featured label="Soil moisture" meterValue={sensors.soilMoisture} value={`${sensors.soilMoisture}%`} /><SensorCard accentColor={AgriColors.temperature} detail="Current reading" label="Temperature" value={`${sensors.temperature}°C`} /><SensorCard accentColor={AgriColors.humidity} detail="Current reading" label="Humidity" value={`${sensors.humidity}%`} /><SensorCard accentColor={AgriColors.warning} detail="FC-37 digital sensor · GPIO 27" label="Rainfall" meterValue={sensors.rainfall} value={sensors.rainfall >= 50 ? 'Wet' : 'Dry'} /><SensorCard accentColor={AgriColors.primary} detail="Simulated demo reading" label="Soil pH" value={typeof sensors.ph === 'number' ? sensors.ph.toFixed(1) : '6.5'} /></View><View style={styles.robotSection}><Text style={styles.sectionKicker}>AGRI BOT LOCATION</Text><Text style={styles.locationHeading}>Track the latest robot position</Text><Pressable onPress={() => router.push('/location')} style={({ pressed }) => [styles.locationButton, pressed && styles.pressed]}><View><Text style={styles.locationTitle}>View robot location</Text><Text style={styles.locationSubtitle}>Open the live position preview</Text></View><Text style={styles.arrow}>→</Text></Pressable></View></ScrollView></SafeAreaView>;
}

function DemoDashboard({ error, logout, onLocation }: { error: string; logout: () => void; onLocation: () => void }) {
  const { sensors } = getMockDashboardData();

  return <SafeAreaView style={styles.screen}><ScrollView contentContainerStyle={styles.content}><View style={styles.header}><ScreenHeader eyebrow="Farmer dashboard" title="AGRI BOT" actionLabel="Log out" onAction={logout} /><Text style={styles.tagline}>Smart farming, simplified.</Text></View><View style={styles.previewBanner}><Text style={styles.previewEyebrow}>DASHBOARD PREVIEW</Text><Text style={styles.previewTitle}>Your farm command centre</Text><Text style={styles.previewText}>Live robot data is temporarily unavailable. These sample cards explain what the dashboard will show once the AgriBot is connected.</Text>{error ? <Text style={styles.previewError}>{error}</Text> : null}</View><View style={styles.statusRow}><View style={styles.statusHeader}><View style={styles.liveLabel}><View style={[styles.statusDot, { backgroundColor: AgriColors.warning }]} /><Text style={[styles.liveText, { color: AgriColors.warning }]}>DEMO</Text></View><Text style={[styles.statusValue, { color: AgriColors.warning }]}>WAITING</Text></View><Text style={styles.statusLabel}>AgriBot connection</Text><Text style={styles.statusHint}>This status will show whether the robot is online and sending updates.</Text></View><View style={styles.sectionHeading}><View><Text style={styles.sectionKicker}>FIELD CONDITIONS</Text><Text style={styles.sectionTitle}>Live environmental readings</Text></View><Text style={styles.updated}>Sample values</Text></View><View style={styles.grid}><SensorCard accentColor={AgriColors.soil} detail="Moisture level in the soil" featured label="Soil moisture" meterValue={sensors.soilMoisture} value={`${sensors.soilMoisture}%`} /><SensorCard accentColor={AgriColors.temperature} detail="Ambient field temperature" label="Temperature" value={`${sensors.temperature}°C`} /><SensorCard accentColor={AgriColors.humidity} detail="Air moisture level" label="Humidity" value={`${sensors.humidity}%`} /><SensorCard accentColor={AgriColors.warning} detail="Rain sensor state" label="Rainfall" meterValue={sensors.rainfall} value="Dry" /><SensorCard accentColor={AgriColors.primary} detail="Acidity of the soil" label="Soil pH" value={`${sensors.ph ?? 6.5}`} /></View><View style={styles.explainerSection}><Text style={styles.sectionKicker}>HOW IT WORKS</Text><Text style={styles.sectionTitle}>What this dashboard will help you do</Text><View style={styles.explainerCard}><Text style={styles.explainerNumber}>01</Text><View style={styles.explainerCopy}><Text style={styles.explainerTitle}>Check field health</Text><Text style={styles.explainerText}>Review moisture, temperature, humidity, rainfall, and pH readings before making watering or crop-care decisions.</Text></View></View><View style={styles.explainerCard}><Text style={styles.explainerNumber}>02</Text><View style={styles.explainerCopy}><Text style={styles.explainerTitle}>Monitor AgriBot</Text><Text style={styles.explainerText}>See whether the robot is connected, when it last reported, and whether live updates are flowing.</Text></View></View><Pressable onPress={onLocation} style={({ pressed }) => [styles.locationButton, pressed && styles.pressed]}><View><Text style={styles.locationTitle}>Robot location preview</Text><Text style={styles.locationSubtitle}>Open the map section when data is available</Text></View><Text style={styles.arrow}>→</Text></Pressable></View></ScrollView></SafeAreaView>;
}
...
const styles = StyleSheet.create({
  ...
  previewBanner: { backgroundColor: AgriColors.primarySoft, borderColor: AgriColors.border, borderRadius: AgriRadius.lg, borderWidth: 1, marginBottom: AgriSpacing.md, padding: AgriSpacing.lg },
  previewEyebrow: { color: AgriColors.primary, fontSize: 11, fontWeight: '900', letterSpacing: 1.3, marginBottom: AgriSpacing.sm },
  previewTitle: { color: AgriColors.text, fontSize: 22, fontWeight: '800', marginBottom: AgriSpacing.xs },
  previewText: { color: AgriColors.textMuted, fontSize: 14, lineHeight: 21 },
  previewError: { color: AgriColors.warning, fontSize: 12, lineHeight: 18, marginTop: AgriSpacing.sm },
  explainerSection: { marginTop: AgriSpacing.xl },
  explainerCard: { alignItems: 'flex-start', backgroundColor: AgriColors.surface, borderColor: AgriColors.border, borderRadius: AgriRadius.md, borderWidth: 1, flexDirection: 'row', marginTop: AgriSpacing.sm, padding: AgriSpacing.md },
  explainerNumber: { color: AgriColors.primary, fontSize: 12, fontWeight: '900', letterSpacing: 1, marginRight: AgriSpacing.md, paddingTop: 2 },
  explainerCopy: { flex: 1 },
  explainerTitle: { color: AgriColors.text, fontSize: 15, fontWeight: '800', marginBottom: 4 },
  explainerText: { color: AgriColors.textMuted, fontSize: 13, lineHeight: 19 },
});
